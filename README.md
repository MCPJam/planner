...
The app is one self-contained HTML resource at `ui://planner/calendar.html`, bundled with MCP Apps SDK. Embedded interactions use `App.callServerTool`; discovery mode dispatches through `execute`. Standalone interactions use the same validated service through `/api/tool`. Dragging tasks or calendar events shows a 15-minute snapped preview with the proposed start, end, and duration. Dropping or dragging the bottom edge opens a confirmation dialog: adjust start/end time, total duration, or a 30/60/90-minute preset, then confirm. Conflicts disable confirmation; Cancel or Escape preserves the original event. Clicking an event opens the same accessible editor. All write paths reject conflicts and protected items.
...
### MCP Apps view tools
The iframe declares the Apps `tools` capability and registers `planner_get_view_state`, `planner_preview_move`, and `planner_preview_replan` with `App.registerTool`. These are app-provided view tools, not WebMCP. View state contains the displayed week and pending confirmation. Move previews open the editor; re-plan previews fetch `plan_week(apply:false)` and display Save/Cancel. Only user confirmation calls a persistent server mutation through `callServerTool` (or `execute` in discovery mode). Context-capable hosts receive debounced UI state via `updateModelContext`. Host support for view-tool discovery varies; ordinary UI controls remain available. `tests/view-tools.test.ts` checks discovery, schema validation, invocation and removal over a real `AppBridge` with linked transports.
